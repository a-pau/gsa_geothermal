...
# Subplots
fig, (cge_ax, ege_ax) = plt.subplots(ncols=2)
...
cge_ticklabels.append("reference model")
...
# Plot
cge_ax.boxplot(x=cge_ref_df, positions=[pos_cge_ref], vert=False, whis=[0,100], showfliers=False,
               widths=1, medianprops={"color":"black"})
sb.scatterplot(data=cge_data, x="carbon footprint", y=pos_cge_lit, style="type", markers=["|"], color="black", s=100, ax=cge_ax)
sb.scatterplot(data=cge_s_df_2, x="carbon footprint", y=pos_cge_s, hue="simplified", ax=cge_ax)
cge_ax.set(xlabel='$g CO_2 eq./kWh$', ylabel = "", yticks=pos_cge_ticks, yticklabels=cge_ticklabels,
           ylim=(pos_cge_ticks[0]-0.5, pos_cge_ticks[-1]+1))
cge_ax.grid(b=True, which='both', axis="x")
cge_ax.xaxis.set_minor_formatter(FormatStrFormatter("%.0f"))
cge_ax.xaxis.set_major_formatter(FormatStrFormatter("%.0f"))
cge_ax.tick_params(axis="x", which="both", labelrotation=45)
cge_ax.set_title("CONVENTIONAL", fontsize=10)
...
ege_ticklabels.append("reference model")
...
ege_ax.boxplot(x=ege_ref_df, positions=[pos_ege_ref], vert=True, whis=[0,100], showfliers=False, 
               widths=1, medianprops={"color":"black"})
sb.scatterplot(data=ege_data, y="carbon footprint", x=pos_ege_lit, style="type", markers=["_"], color="black", ax=ege_ax)
...
#%% Save
fig.savefig(os.path.join(folder_OUT, file_name + ".tiff"), dpi=600)
