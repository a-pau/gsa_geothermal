...
# Parameters
# Hellisheidi
cge_parameters = {"co2_emissions": 20.9/1000,
...
# UDDGP
ege_parameters = {"installed_capacity": 1,
...
cge_ref_df = pd.read_json(os.path.join(folder_IN, file_name_IN + "_Conventional_Reference")).melt(var_name="method", value_name="impact")
ege_ref_df = pd.read_json(os.path.join(folder_IN, file_name_IN + "_Enhanced_Reference")).melt(var_name="method", value_name="impact")
...
HSD_scores = pd.read_excel(os.path.join(folder_IN, "Hellisheidi impacts.xlsx"), sheet_name="Sheet1", index_col=0, 
                           usecols=[0,3,4]).rename(columns={"method_3":"method"})
...
if exploration:
    ...
    ege_plot.savefig(os.path.join(folder_OUT, file_name_ege + "_NO-EXPL" +".tiff"), dpi=300)
