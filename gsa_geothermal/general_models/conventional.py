...
class GeothermalConventionalModel:
    def __init__(
        ...
        exploration=True,
        success_rate=True,
        ecoinvent_version="ecoinvent 3.6 cutoff",
    ):
        ...
        (
            ...
            _,
            self.co2,
            self.electricity_prod,
            _,
        ) = lookup_geothermal(ecoinvent_version=ecoinvent_version)
        # kilogram (for open hole diameter of 8.5 in and assume factor 3 production liner to total volume drilled)
    ...
    def model(self, parameters):
        ...
        success_rate_exploration_wells = (
            parameters["success_rate_exploration_wells"] / 100
        ) / self.success_rate_opt[0]
        success_rate_primary_wells = (
            parameters["success_rate_primary_wells"] / 100
        ) / self.success_rate_opt[1]
        success_rate_makeup_wells = (
            parameters["success_rate_makeup_wells"] / 100
        ) / self.success_rate_opt[2]
        ...
        number_of_production_wells = np.ceil(
            parameters["installed_capacity"] / parameters["gross_power_per_well"]
        )  # Total number of wells, rounded up

        number_of_injection_wells = np.ceil(
            number_of_production_wells / parameters["production_to_injection_ratio"]
        )

        number_of_makeup_wells = np.ceil(
            number_of_production_wells
            * parameters["initial_harmonic_decline_rate"]
            * parameters["lifetime"]
        )

        number_of_wells_wo_expl = (
            number_of_production_wells
            + number_of_injection_wells
            + number_of_makeup_wells
        )

        total_collection_pipelines = (
            number_of_wells_wo_expl * parameters["collection_pipelines"]
        )  # metres

        number_of_wells = (
            number_of_wells_wo_expl
            + self.number_of_expl_wells * self.equivalency_expl_to_normal_wells
        )

        number_of_wells_drilled = (
            np.ceil(
                (number_of_production_wells + number_of_injection_wells)
                / success_rate_primary_wells
            )
        ...
        total_metres_drilled = (
            number_of_wells_drilled * parameters["average_depth_of_wells"]
        )  # metres
        ...
        drilling_waste = (
            -total_metres_drilled * self.drilling_waste_per_metre
        )  # kilogram (minus because waste)

        # self.number_of_wells, self.diesel_consumption, self.steel_consumption, self.cement_consumption, \
        # self.water_cem_consumption, self.drilling_mud_consumption, self.drilling_waste, self.total_metres_drilled,
        # self.total_collection_pipelines, self.lifetime_electricity_generated = \
    ...
    def run_with_presamples(self, parameters):
        ...
        return [
            (
                self.wellhead,
                self.electricity_prod,
                np.array(number_of_wells / lifetime_electricity_generated),
            ),
            (
                self.diesel,
                self.electricity_prod,
                np.array(diesel_consumption / lifetime_electricity_generated),
            ),
            (
                self.steel,
                self.electricity_prod,
                np.array(steel_consumption / lifetime_electricity_generated),
            ),
            (
                self.cement,
                self.electricity_prod,
                np.array(cement_consumption / lifetime_electricity_generated),
            ),
            (
                self.water,
                self.electricity_prod,
                np.array(water_cem_consumption / lifetime_electricity_generated),
            ),
            (
                self.drilling_mud,
                self.electricity_prod,
                np.array(drilling_mud_consumption / lifetime_electricity_generated),
            ),
            (
                self.drill_wst,
                self.electricity_prod,
                np.array(drilling_waste / lifetime_electricity_generated),
            ),
            (
                self.wells_closr,
                self.electricity_prod,
                np.array(total_metres_drilled / lifetime_electricity_generated),
            ),  # well closure
            (
                self.coll_pipe,
                self.electricity_prod,
                np.array(total_collection_pipelines / lifetime_electricity_generated),
            ),
            (
                self.plant,
                self.electricity_prod,
                np.array(
                    parameters["installed_capacity"] / lifetime_electricity_generated
                ),
            ),
