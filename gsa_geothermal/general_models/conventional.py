...
class GeothermalConventionalModel:
    ...
    def __init__(self, parameters, exploration=True, success_rate=True, ecoinvent_version="ecoinvent 3.6 cutoff"):
        ...
        self.wellhead, self.diesel, self.steel, self.cement, self.water, self.drilling_mud, self.drill_wst, \
            self.wells_closr, self.coll_pipe, self.plant, _, _, _, self.co2, self.electricity_prod, _ \
    ...
    def get_input_output_array(self):
        ...
        inputs = np.array(
            [
                self.wellhead, self.diesel, self.steel, self.cement, self.water, self.drilling_mud, self.drill_wst,
                self.wells_closr, self.coll_pipe, self.plant, self.co2
            ]
        )
    ...
    def model(self, parameters):
        ...
        number_of_makeup_wells = np.ceil(
            number_of_production_wells * parameters["initial_harmonic_decline_rate"] * parameters["lifetime"]
        )
        
        number_of_wells_wo_expl = number_of_production_wells + number_of_injection_wells + number_of_makeup_wells
        ...
        number_of_wells_drilled = (
            np.ceil((number_of_production_wells + number_of_injection_wells) / success_rate_primary_wells) +
        ...
        steel_consumption = total_metres_drilled * parameters["specific_steel_consumption"]  # kilogram

        cement_consumption = total_metres_drilled * parameters["specific_cement_consumption"]  # kilogram
