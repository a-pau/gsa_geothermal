...
class GeothermalEnhancedModel:
    ...
    def model(self, parameters):
        ...
        number_of_wells = parameters["number_of_wells"] + (
        ...
        total_collection_pipelines = (
            parameters["number_of_wells"] * parameters["collection_pipelines"]
        )  # metres
        ...
        number_of_wells_stim = np.round(
            0.5
            + parameters[  # So that at least one well is stimulatd
        ...
        diesel_for_stim = (
            (water_stim_consumption / 1000)
            * parameters["specific_electricity_stimulation"]
            * 3.6
    ...
    def run_with_presamples(self, parameters):
        ...
        return [
            ...
            (
                self.water,
                self.electricity_prod,
                np.array(total_water_consumption / lifetime_electricity_generated),
            ),
            (
                self.drilling_mud,
                self.electricity_prod,
            ...
            (
                self.ORC_fluid,
                self.electricity_prod,
                np.array(ORC_fluid_consumption / lifetime_electricity_generated),
            ),
            (
                self.ORC_fluid_wst,
                self.electricity_prod,
            ...
            (
                self.diesel_stim,
                self.electricity_prod,
                np.array(diesel_for_stim / lifetime_electricity_generated),
            ),
        ]
