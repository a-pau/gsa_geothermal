...
def convert_sample_to_normal_or_lognormal(parameters, sample, distribution):
    ...
    q_low_ppf  = distribution.ppf(parameters[mask_is_neg_inf], np.array([Q_LOW] * mask_is_neg_inf.shape[0]))
    q_high_ppf = distribution.ppf(parameters[mask_is_pos_inf], np.array([Q_HIGH] * mask_is_pos_inf.shape[0]))
...
def convert_sample_to_truncated_triang(parameters, sample):
    ...
    q_low_t  = stats.triang.cdf(parameters["minimum"], c=parameters["shape"], loc=parameters["loc"], scale=parameters["scale"])
    q_high_t = stats.triang.cdf(parameters["maximum"], c=parameters["shape"], loc=parameters["loc"], scale=parameters["scale"])
    q_t = (q_high_t - q_low_t)*sample + q_low_t
    params_converted = stats.triang.ppf(q_t, c=parameters["shape"], loc=parameters["loc"], scale=parameters["scale"])
    return params_converted
...
def convert_sample(parameters, sample):
    ...
    if distr == sa.NormalUncertainty or distr == sa.LognormalUncertainty:
        ...
        converted[loc_not_nan] = convert_sample_to_truncated_triang(parameters[loc_not_nan], sample[loc_not_nan]).flatten()
        return converted
    else:
