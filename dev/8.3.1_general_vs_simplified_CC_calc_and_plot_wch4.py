...
for t in threshold_cge:
    ...
    for _, rows in cge_data.iterrows():
        ...
        temp_[rows["study"]] = (
            cge_model_s[t].run(rows, lcia_methods=ILCD_CC, ch4=True)[ILCD_CC[0][-1]]
            * 1000
        )
    cge_s[t] = temp_
...
# Subplots
fig, cge_ax = plt.subplots(nrows=1, ncols=1, sharex="col")
...
cge_ax.set_ylabel("$\mathregular{g CO_2 eq./kWh}$")
...
#%% Plot only conventional with break axis - SUBSIDED
...
# Subplots
fig, (cge_ax_up, cge_ax_low) = plt.subplots(
    nrows=2, ncols=1, sharex="col", gridspec_kw={"height_ratios": [1, 2]}
)
