...
for counter, method_ in enumerate(ILCD):
    if counter <= 7:
        ...
        i= counter-8
        j=1
    sb.boxplot(data=cge_ref_df[cge_ref_df.method_3==method_[2]], x="method_3", y="impact score", hue="model", color="white", showfliers=False, whis=[1,99], ax=cge_ax[j][i])
    sb.stripplot(data=cge_s_and_Hellisheidi_df[cge_s_and_Hellisheidi_df.method_3==method_[2]], x="method_3", y= "impact score", hue="type",jitter=False, dodge=True, s=7, ax=cge_ax[j][i])
...
for counter, method_ in enumerate(ILCD):
    if counter <= 7:
        ...
        i= counter-8
        j=1
    sb.boxplot(data=ege_ref_df[cge_ref_df.method_3==method_[2]], x="method_3", y="impact score", hue="model", color="white", showfliers=False, whis=[5,95], ax=ege_ax[j][i])
    sb.stripplot(data=ege_s_and_UDDGP_df[ege_s_and_UDDGP_df.method_3==method_[2]], x="method_3", y= "impact score", hue="type",jitter=False, dodge=True, s=7, ax=ege_ax[j][i])
...
handles, _ = ege_ax[0][0].get_legend_handles_labels()
labels=["general model", "simplified model", "UDDGP"]
ege_plot.legend(handles, labels, loc='upper left', ncol=3) 
